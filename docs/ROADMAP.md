...
The capability framework remains in progress until reusable middleware
contracts, lifecycle, availability, health, and additional capability
...
1. Add capability middleware

This priority will add ordered reusable interception around the delivered
provider-neutral execution pipeline while preserving the existing
`ChatCapability` JSON, streaming, timeout, cancellation, error, logging,
tracing, context, cleanup, and discovery behavior. Lifecycle, routing, retry,
provider registration, and dynamic plugins will remain separate milestones.
